...
void AddRoundKey(uint8 stat[], uint32 W[], int round)
{
	...
	for (int k = 0; k < NB;k++)
	{
		...
		PUT_UINT32(W[NB*round+k],temp,0);
		for (int i = 0;i < 4;i++)
			stat[k * 4 + i] ^= temp[i];
...
void MixColumns(uint8 stat[])
{
	...
	for (int i = 0; i < NB;i++)
	{
		...
		stat[i * 4 + 0] = dot(0x02, temp[i * 4]) ^ dot(0x03, temp[i * 4 + 1]) ^ temp[i * 4 + 2] ^ temp[i * 4 + 3];
		stat[i * 4 + 1] = temp[i * 4] ^ dot(0x02, temp[i * 4 + 1]) ^ dot(0x03, temp[i * 4 + 2])^temp[i * 4 + 3];
		stat[i * 4 + 2] = temp[i * 4] ^ temp[i * 4 + 1] ^ dot(0x02, temp[i * 4 + 2]) ^ dot(0x03, temp[i * 4 + 3]);
		stat[i * 4 + 3] = dot(0x03, temp[i * 4]) ^ temp[i * 4 + 1] ^ temp[i * 4 + 2] ^ dot(0x02, temp[i * 4 + 3]);
